...
#define BUFFER_SIZE ((int)SAMPLING_RATE) // 一秒数据的缓冲区大小
...
int main() {
    ...
    for (int i = 0; i < NUM_SAMPLES; ++i) {
        ...
        float sum_X = 0.0f;
        float sum_Y = 0.0f;
        int count = buffer_filled ? BUFFER_SIZE : (buffer_index + 1);
        ...
        current_filtered_X = sum_X / count;
        current_filtered_Y = sum_Y / count;
