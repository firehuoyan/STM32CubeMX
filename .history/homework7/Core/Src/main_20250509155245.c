...
int main(void)
{
  ...
  uint8_t task = 0x00;  // 0x00呼吸。0xFF彩虹. Default to breathing light
  uint16_t omega = 1;    // Default speed (1-255). Min value 1.
  
  uint32_t breathing_time_step = 0; // Time step for sine wave calculation
  
  uint8_t rainbow_current_index = 0; // Current index in the rainbow_colors table
  uint32_t rainbow_tick_counter = 0;  // Counter for rainbow color switching delay
  ...
  if (!FIFO_In_IsEmpty()) // 检查输入FIFO是否有数据
    {
      ...
      if (fifo_in.wParam == 0x00 or fifo_in.wParam == 0xFF) // 判断任务
        {
