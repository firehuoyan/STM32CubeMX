...
int main(void)
{
  ...
  if (task == 0xFF) {
    ...
    // 计算颜色切换的延迟。
    uint16_t switch_delay_ms = 5000 / omega; 
    if (switch_delay_ms < 1) { // 
        switch_delay_ms = 1; 
    }
    ...
    __HAL_TIM_SetCompare(&htim5, TIM_CHANNEL_1, r_val/2); // 设置R通道PWM占空比
    __HAL_TIM_SetCompare(&htim5, TIM_CHANNEL_2, g_val/2); // 设置G通道PWM占空比
    __HAL_TIM_SetCompare(&htim5, TIM_CHANNEL_3, b_val/2); // 设置B通道PWM占空比
  }
