...
int main(void)
{
  ...
  if (task == 0x00) {
    ...
    float time_factor = (float)breathing_time_step * (float)omega * 0.0005f; 

    // 使用具有相移的正弦波计算 R, G, B 值
    // (sin(x) + 1)/2 将 sin 输出从 [-1,1] 映射到 [0,1]
    float r_norm = (sinf(time_factor) + 1.0f) / 2.0f;
    float g_norm = (sinf(time_factor + (2.0f * M_PI / 3.0f)) + 1.0f) / 2.0f; // G通道相位偏移 2pi/3
    float b_norm = (sinf(time_factor + (4.0f * M_PI / 3.0f)) + 1.0f) / 2.0f; // B通道相位偏移 4pi/3

    // 将归一化值缩放到 PWM 范围
    uint16_t pwm_r = (uint16_t)(r_norm * PWM_MAX_VALUE);
    uint16_t pwm_g = (uint16_t)(g_norm * PWM_MAX_VALUE);
    uint16_t pwm_b = (uint16_t)(b_norm * PWM_MAX_VALUE);

    __HAL_TIM_SetCompare(&htim5, TIM_CHANNEL_1, pwm_r); // 设置R通道PWM占空比
    __HAL_TIM_SetCompare(&htim5, TIM_CHANNEL_2, pwm_g); // 设置G通道PWM占空比
    __HAL_TIM_SetCompare(&htim5, TIM_CHANNEL_3, pwm_b); // 设置B通道PWM占空比
  }
  if (task == 0xFF) {
