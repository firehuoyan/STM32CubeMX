...
void send_f(UART_HandleTypeDef *huart, float value, uint32_t delay)
{
  char buffer[20];
  sprintf(buffer, "%.2f", value);  // 显示1位小数
...
uint16_t adcx_get_chx_value(ADC_HandleTypeDef *ADCx, uint32_t channel)
{
    ...
    HAL_ADC_PollForConversion(ADCx, 1);
...
int main(void)
{
  ...
  sprintf(adc_str, "ADC Value with calib: %.2f\r\n", adcx_get_u(&hadc1, ADC_CHANNEL_VREFINT));

  for (int i = 0; i<100; i++)
  {
    send_f(&huart1, adcx_get_u(&hadc1, ADC_CHANNEL_VREFINT), 10);
    HAL_Delay(1);
  }
