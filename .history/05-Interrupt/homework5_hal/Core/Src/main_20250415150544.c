...
static uint32_t btn_a_last_click_time = 0;   
static uint8_t btn_a_click_count = 0;  

static uint32_t btn_b_press_start_time = 0;  
static uint8_t btn_b_is_pressed = 0;  
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  if(GPIO_Pin == KEY_B_Pin)
  {
    ...
    if(pin_state == 1) // 按键按下 (低电平有效)
    {
      // 记录按下开始时间
      if(btn_b_is_pressed == 0)
      {
        btn_b_press_start_time = current_time;
        btn_b_is_pressed = 1;
        PostMessage(MSG_BTNDOWN, BTN_B, 0);
      }
    }
    else // 按键释放
    {
      if(btn_b_is_pressed == 1)
      {
        ...
        // 计算按下持续时间，判断是否为长按
        uint32_t press_duration = current_time - btn_b_press_start_time;
        if(press_duration >= BTN_REPEAT_DELAY)
        {
          // 长按释放，发送一个MSG_CHAR消息
          PostMessage(MSG_CHAR, BTN_B, press_duration);
        }
      }
    }
  }
