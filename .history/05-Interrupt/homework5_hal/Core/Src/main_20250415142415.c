...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  // 消抖
  if(current_time - last_key_time > 100) // 注意：这里使用更合理的100ms而非1000ms
  {
    last_key_time = current_time;
  }
  else
  {
    return; // 如果时间间隔太短，直接返回不处理
  }

	//按下A
	if(GPIO_Pin==KEY_A_Pin)
	{
    // 确保从上次按键状态变化后至少经过100ms
    if(current_time - last_key_a_time > 1000)
    {
        last_key_a_time = current_time;
        uint8_t pin_state = HAL_GPIO_ReadPin(KEY_A_GPIO_Port,KEY_A_Pin);
        PostMessage(pin_state ? MSG_BTNDOWN : MSG_BTNUP, BTN_A, 0);
    }
    // HAL_Delay(10000);
    // uint8_t pin_state = HAL_GPIO_ReadPin(KEY_A_GPIO_Port,KEY_A_Pin);
    // PostMessage(pin_state ? MSG_BTNDOWN : MSG_BTNUP, BTN_A, 0);
