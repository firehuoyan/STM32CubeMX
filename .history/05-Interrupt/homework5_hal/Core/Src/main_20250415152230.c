...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  if(GPIO_Pin == KEY_B_Pin)
  {
      ...
      if(pin_state == 1) // 按键 B 按下 (假设低电平有效)
      {
        // 仅在状态从未按下变为按下时执行
        if(btn_b_is_pressed == 0)
        {
          ...
          PostMessage(MSG_BTNDOWN, BTN_B, 0);       // 发送按下消息
        }
      }
      else // 按键 B 释放
      {
        ...
        {
          btn_b_is_pressed = 0;                     // 清除按下状态
          PostMessage(MSG_BTNUP, BTN_B, 0);         // 发送抬起消息
          // 注意：长按的重复消息发送逻辑在定时器中处理，这里不需要做额外操作
        }
      }
  }
